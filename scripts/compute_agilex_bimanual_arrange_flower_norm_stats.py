"""Compute norm stats for the AgileX bimanual arrange-flower dataset."""
...
def main() -> None:
    config = _config.get_config("pi05_agilex_bimanual_arrange_flower_finetune")
    dataset_dir = Path(config.data.repo_id)
    asset_id = config.data.assets.asset_id
    if asset_id is None:
        raise ValueError("Expected pi05_agilex_bimanual_arrange_flower_finetune to define an asset_id")
    ...
    for parquet_file in tqdm.tqdm(parquet_files, desc="Computing AgileX bimanual arrange-flower stats"):
        ...
        if states.shape[-1] != 14 or actions.shape[-1] != 14:
            raise ValueError(f"Expected 14-dim state/action in {parquet_file}, got {states.shape} and {actions.shape}")
